...
def load_data(lang):
    text_data = list()
    code_data = list()
    prefix = '/data01/lizehan/proqa/raw_data/conala'
    text_file = f'{prefix}/conala-test-curated-0.5.jsonl'
    code_file = f'{prefix}/conala-curated-snippets.jsonl'
...
def evaluate(model, data):
    ...
    text_queries = [text['query'] for text in text_data]
    code_corpus = [code['description'] + " " + code['code'] for code in code_data]
    ...
    text_urls = [text['relevant_ids'] for text in text_data]
    code_urls = [code['id'] for code in code_data]

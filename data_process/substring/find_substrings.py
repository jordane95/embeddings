...
class SubstringFilterer(object):
    ...
    def save(self, filtered, num_proc):
        # Save filtered to disk
        save_path = os.path.join(self.data_dir, "decontanimated_qa.jsonl")
...
def main():
    ...
    data_path = '/data01/lizehan/proqa/pls/qa.en.python.json'

    ds = load_dataset(
        'json', data_files=args.input_path, split="train[:80%]",
    )

    ds = ds.map(lambda x: {'content': ' '.join([x['title'], x['question'], x['answer']]), 'lang': args.lang})

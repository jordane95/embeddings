...
from utils import logger, pool, move_to_cuda, remove_comments_and_docstrings
...
def load_data(lang):
    text_data = list()
    code_data = list()
    prefix = '/data01/lizehan/proqa/code/CodeXGLUE/Text-Code/NL-code-search-Adv/dataset'
    text_file = f'{prefix}/test.jsonl'
    code_file = f'{prefix}/test_code.jsonl'
...
def evaluate(model, data):
    ...
    text_queries = [' '.join(text['docstring'].split('\n\n')[0].split()) for text in text_data]
    code_corpus = [remove_comments_and_docstrings(code['function'], 'python') for code in code_data]

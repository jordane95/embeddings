...
class DenseEncoder(torch.nn.Module):
    ...
    def encode(self, sentences, **kwargs) -> np.ndarray:
        ...
        results = torch.cat(encoded_embeds, dim=0)
        return results
...
def evaluate(model, data):
    ...
    for lang, code_data in all_data.items():
        code_snippets = [' '.join(remove_comments_and_docstrings(code['func'], lang).split()) for code in code_data]
    ...
    def evaluate_lang(src_lang, tgt_lang):
        ...
        MAP = []
        results = {}
        for i in tqdm.tqdm(range(scores.shape[0])):
            cont = 0

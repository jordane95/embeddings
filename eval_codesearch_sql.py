...
def load_data(lang):
    text_data = list()
    code_data = list()
    data_file = '/data01/lizehan/proqa/code/CDCS/dataset/test/{}.txt'.format(lang)
...
def evaluate(model, data):
    ...
    result = {
        "mrr@1000": float(np.mean(rr)),
    }
    for k in hit_k:
        result["hit@{}".format(k)] = float(hit_k[k]) / len(ranks)

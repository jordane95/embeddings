...
def load_data(lang):
    text_data = list()
    code_data = list()
    prefix = '/data01/lizehan/proqa/code/CoCLR/data/search'
    text_file = f'{prefix}/cosqa-retrieval-test-500.json'
    code_file = f'{prefix}/code_idx_map.txt'
    text_data: List[Dict] = json.load(open(text_file))

    code_data = []
    for code, idx in json.load(open(code_file)).items():
        code_data.append({"code": code, "idx": idx})
...
def evaluate(model, data):
    ...
    text_queries = [text['doc'] for text in text_data]
    code_corpus = [code['code'] for code in code_data]
    ...
    text_urls = [text['retrieval_idx'] for text in text_data]
    code_urls = [code['idx'] for code in code_data]
...
def main():
    ...
    with open(os.path.join(args.output_dir, "avg.json"), 'w') as f:
        json.dump({"mrr": scores / len(langs)}, f, indent=2)
